...
const AdminHome = (props) => {
  ...
  const calculateSalesForDayAndYesterday = (orders) => {
    ...
    const percentageChange =
      ((salesForToday - salesForYesterday) / salesForYesterday) * 100;
  ...
  const calculateWeeklySales = (orders) => {
    ...
    const currentWeekSales = orders
      ...
      .reduce(
        (weeklySalesData, order) => {
          const orderDate = new Date(order.issueDate);
          const dayOfWeek = orderDate.getDay();

          weeklySalesData[dayOfWeek] =
            (weeklySalesData[dayOfWeek] || 0) + order.totalCost;
    ...
    const prevWeekSales = orders
      ...
      .reduce(
        (prevWeekSalesData, order) => {
          const orderDate = new Date(order.issueDate);
          const dayOfWeek = orderDate.getDay();

          prevWeekSalesData[dayOfWeek] =
            (prevWeekSalesData[dayOfWeek] || 0) + order.totalCost;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const fetchedOrders = await fetchAllOrders();
        const { currentWeekSales, prevWeekSales } =
  ...
  return (
    <div>
      {showDeleteConfirmationModal && (
        <ConfirmationModal
          exit={() => setShowDeleteConfirmationModal(false)}
          confirm={() => {
            handleDeleteClick();
      ...
      {showRejectConfirmationModal && (
        <ConfirmationModal
          exit={() => setShowRejectConfirmationModal(false)}
          confirm={() => {
            reject();
      ...
      <Container className={styles.sales}>
        <h2 className={styles.salesTitle} onClick={handleSalesClick}>
          Sales Report
        </h2>
              ...
              <tbody>
                {requests.map((request) => (
                  <tr
                    ...
                    onClick={() => showRequestModal(request)}
                  >
                    ...
                    <td className={styles.bold}>
                      ...
                      <div className={styles.small}>
                        ...
                        {request.affiliation} {formatDate(request.creationDate)}
                      </div>
                    </td>
                ...
                <ActionButtons reject={() => reject(request)} accept={() => accept(request)} />
                    ...
                    <td
                      className={`${styles.rejectReq} ${styles.borderBottom}`}
                    ...
                    <td
                      className={`${styles.acceptReq} ${styles.borderBottom}`}
                      onClick={(e) => e.stopPropagation()}
                    >
                      <img
                        src={check}
        ...
        <div className={styles.filter}>
          ...
          <button
            className={`${styles.filterButton} ${
              activeRole === 'patient' ? styles.active : ''
            }`}
          ...
          <button
            className={`${styles.filterButton} ${
              activeRole === 'pharmacist' ? styles.active : ''
            }`}
          ...
          <button
            className={`${styles.filterButton} ${
              activeRole === 'admin' ? styles.active : ''
            }`}
                  ...
                  >
                    ...
                    {activeRole === 'patient' && (
                      <>
                        ...
                        <td>
                          {user.package ? user.package.name : 'No Subscription'}
                        </td>
                        <td>{user.email}</td>
      ...
      <Container className={styles.addAdmin}>
        <button className={styles.adminButton} onClick={toggleAddForm}>
          Add Admin
        </button>
      </Container>
      {selectedUser && (
